...
public class AdvancedJDBCRealmTest extends BaseTestCase {
    ...
    public void doAuthorizationStuff() throws Exception {
        ...
        try {
          ...
          // exptected error in negative testing
            assertTrue(e.getCause().getCause().getCause() instanceof IllegalArgumentException);
                if (log.isDebugEnabled()) {
                    log.debug("Expected error, hence ignored", e);
                }
        }
...
public void doClaimStuff() throws Exception {
        ...
        try {
            ...
            fail("Exception at get user claim value for null user");
        } catch (UserStoreException e) {
            assertTrue("Exception at get user claim value for null user", e instanceof UserStoreException && e
                    .getMessage().contains("UserNotFound"));
            if (log.isDebugEnabled()) {
                log.debug("Expected error, hence ignored", e);
            }
        }
